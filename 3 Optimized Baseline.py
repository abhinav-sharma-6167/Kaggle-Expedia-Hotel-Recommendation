...
def prepare_arrays_match():
    ...
    while 1:
        ...
        append_0 = ((book_year - 2013)*12 + (book_month - 12))
        ...
        append_1 = pow(math.log(append_0), 1.32) * (-0.1+1.09*pow(append_0, 1.5)) * (3.5 + 17.62*is_booking)
        append_2 = 3.12 + 5.565*is_booking
...
def gen_submission(best_s00, best_s01,best_hotels_country, best_hotels_search_dest, best_hotels_od_ulc, best_hotels_uid_miss, popular_hotel_cluster):
    ...
    path = 'my_submission_' + str(now.strftime("%Y-%m-%d-%H-%M")) + '.csv'
    out = open(path, "w")
    f = open("test.csv", "r")
    f.readline()
    total = 0
