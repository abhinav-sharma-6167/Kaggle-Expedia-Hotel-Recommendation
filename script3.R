...
#dont just loop around..subsetting cooler
...
k<-as.data.frame(table(new_train_booking$arr_day,new_train_booking$hotel_cluster))
k<-as.data.table(k)
...
k<-as.data.frame(table(new_train_booking$arr_day,new_train_booking$hotel_cluster))
k<-as.data.table(k)
new_train_booking$setkey1 <- 1000*(as.numeric(new_train_booking$arr_day)-1)+as.numeric(new_train_booking$hotel_cluster)
k$setkey1 <-1000*(as.numeric(k$Var1)-1)+as.numeric(k$Var2)-1
setkey(new_train_booking,setkey1)
setkey(k,setkey1)
new_train_booking<-merge(new_train_booking,k)
  ...
  #function over prediction and xgboost object
{
  ...
  top <- t(apply(preds,1,  function(y) order(y)[num.class:(num.class-4)]-1))
...
model_xgb2 <- xgb.train(param, train.xg, nthread = 8, nround = 250, watchlist,
...
prediction1<-predict(model_xgb2,validation2, missing=NaN)
...
#while evaluating error metric
#first evaluating labels to be used in assigning which label is for wt hotel cluster
#
...
j<-1:length(unique(new_train_booking$hotel_cluster))
for(j in 1:length(unique(new_train_booking$hotel_cluster))){
  c1 <- subset(new_train_booking, subset=hotel_cluster==j, select = c(26,27))
  #c2<-as.data.frame(unique(c1))
  c3 <- c1[,.N,by=arr_day]



  ggplot(c3, aes(x = arr_day, y = N)) + geom_point()
  ggsave(file=paste0(j,".png"))
